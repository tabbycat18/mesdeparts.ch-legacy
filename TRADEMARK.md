# Trademark Notice

"mesdeparts.ch", including associated names, logos, and branding,
are trademarks (or claimed marks) of Mattia Pastore.

The Apache-2.0 license for this repository grants rights to the source code,
but does not grant rights to use project names, logos, or branding, except as
required for reasonable and customary attribution.

If you want to use these marks beyond nominative/reference use, request written
permission from the trademark owner.
