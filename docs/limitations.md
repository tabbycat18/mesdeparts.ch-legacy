...
`mesdeparts.ch-legacy` is an older public codebase for MesDeparts. It preserves a legacy public client and related public code surfaces, but it is not the full current live MesDeparts system.
...
- full feature parity with the current MesDeparts service
...
- Behavior may differ from what users see on the current live MesDeparts service.
...
- The repo should not be read as a commitment to reproduce the current MesDeparts production stack in public.

## Non-Goals

This repository is not trying to become:

- the full current MesDeparts production stack
